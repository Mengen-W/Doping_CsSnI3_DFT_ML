...
### Reading training data (E_form for X_Cs and X_Sn) ###

data = pd.read_csv('../Dataset/Training dataset_X_Cs_and_X_Sn_Ef(1).csv')

training_data = data.to_numpy()
dopant = training_data[0:, 0]
feature = training_data[0, 1:-1]
E_form = training_data[0:, -1]
...
feature_names = ['EA',  '1st IE', '2nd IE', '3rd IE', 'D', 't (AR) 100%', 'OS', 'HF (exp)', 'S', 'HV']
...
ax.scatter(E_form_test[:], Pred_test[:], c='#615EFC', marker='o', s=60, label='Test')
ax.scatter(E_form_train[:], Pred_train[:], c='#FA7070', marker='o', s=60, label='Train')
...
ax.set_yticks([-0.5, 0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0])
...
### Reading out-of-sample data and predict E_form ###

read_data_out = pd.read_csv('../Dataset/Testing dataset_X_Cs.csv')
...
Pred_E_form = rfreg_opt.predict(X_out_fl)

### Open a text file to save the predictions ###

with open('Predictions_RFR_E_form(1)_X_Cs.txt', 'w') as f:
    f.write("Dopant, Predicted Formation Energy\n")
    
    for i in range(n_out):
        f.write(f"{dopant_out[i]}, {Pred_E_form[i]:.4f}\n")

print("Predictions have been saved to 'Predictions_RFR_E_form(1)_X_Cs.txt'.")
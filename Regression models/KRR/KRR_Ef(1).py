...
### Reading training data (E_form for X_Sn) ###

data = pd.read_csv('../Dataset/Training dataset_Ef(1).csv')
...
'''
best_kernel = ExpSineSquared(length_scale=0.599, periodicity=35.9)
best_alpha = 1e-1  
...
Pred_E_form = krreg_opt.predict(X_out_fl)

### Open a text file to save the predictions ###

with open('Predictions_KRR_E_form(1).txt', 'w') as f:
    f.write("Dopant, Predicted Formation Energy\n")
    
    for i in range(n_out):
        f.write(f"{dopant_out[i]}, {Pred_E_form[i]:.4f}\n")

print("Predictions have been saved to 'Predictions_KRR_E_form(1).txt'.")
...
'''
best_kernel = DotProduct(sigma_0=1)
best_alpha = 1.0
best_n_restarts_optimizer = 100
...
Pred_E_form = gpreg_opt.predict(X_out_fl)

### Open a text file to save the predictions ###

with open('Predictions_GPR_E_form(1).txt', 'w') as f:
    f.write("Dopant, Predicted Formation Energy\n")
    
    for i in range(n_out):
        f.write(f"{dopant_out[i]}, {Pred_E_form[i]:.4f}\n")

print("Predictions have been saved to 'Predictions_GPR_E_form(1).txt'.")
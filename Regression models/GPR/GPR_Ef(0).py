...
data = pd.read_csv('../Dataset/Training dataset_Ef(0).csv')
...
'''
best_kernel = Matern(length_scale=2.15e+03, nu=2)
best_alpha = 0.1
best_n_restarts_optimizer = 250
...
with open('Predictions_GPR_E_form(0).txt', 'w') as f:
    f.write("Dopant, Predicted Formation Energy\n")
    
    for i in range(n_out):
        f.write(f"{dopant_out[i]}, {Pred_E_form[i]:.4f}\n")

print("Predictions have been saved to 'Predictions_GPR_E_form(0).txt'.")